...
.performance <- function(data, estvarname, se, true, empse_ref = NULL, rho = NULL, ci.limits, control) {
  ...
  nsim <- sum(!is.na(data[[estvarname]]) &
    !is.na(data[[se]]), na.rm = control$na.rm)
  ...
  power <- 1 / nsim * sum(abs(data[[estvarname]]) >= crit * data[[se]], na.rm = control$na.rm)
  ...
  if (control$mcse) {
    ...
    if (!is.null(true)) cover_mcse <- sqrt(cover * (1 - cover) / nsim)
  ...
  if (control$mcse) {
    obj$mcse <- c(rep(NA, 5), bias_mcse, empse_mcse, mse_mcse, relprec_mcse, modelse_mcse, relerror_mcse, cover_mcse, becover_mcse, power_mcse)
  }
