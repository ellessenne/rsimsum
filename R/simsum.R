...
simsum <-
	function(data,
		...
		checkmate::assert_choice(modelsemethod,
														 choices = c("rmse", "mean"),
														 add = arg_checks)
		...
		if (dropbig) {
			...
			big_estvarname = data.frame(rownumber = which((data[[estvarname]] - mean(data[[estvarname]])) / sqrt(stats::var(data[[estvarname]])) >= max),
																	value = data[[estvarname]][(data[[estvarname]] - mean(data[[estvarname]])) / sqrt(stats::var(data[[estvarname]])) >= max])
			big_se = data.frame(rownumber = which(data[[se]] >= mean(data[[se]]) * semax),
		...
		if (is.null(by)) {
			...
			if (is.null(methodvar)) {
				...
				ncorr = vapply(
					X = methods,
					FUN = function(x)
						sum(!is.na(methodvar_split[[ref]][[estvarname]]) &
									!is.na(methodvar_split[[x]][[estvarname]])),
					FUN.VALUE = numeric(1)
				)
				summ = lapply(
					X = methods,
			...
			summ = lapply(seq_along(by_split), function(i) {
				...
				if (is.null(methodvar)) {
					...
					ncorr = vapply(
						...
						FUN = function(x)
							sum(
								!is.na(methodvar_split[[ref]][[estvarname]]) &
									!is.na(methodvar_split[[x]][[estvarname]])
							),
						FUN.VALUE = numeric(1)
					)
...
perfms <-
	function(data,
		...
		bsims = sum(!is.na(data[[estvarname]]), na.rm = na.rm)
		sesims = sum(!is.na(data[[se]]), na.rm = na.rm)
		bothsims = sum(!is.na(data[[estvarname]]) &
									 	!is.na(data[[se]]), na.rm = na.rm)
		# Mean, median and variance of betas
		beta_mean = mean(data[[estvarname]], na.rm = na.rm)
		beta_median = stats::median(data[[estvarname]], na.rm = na.rm)
		beta_var = stats::var(data[[estvarname]], na.rm = na.rm)
		...
		esd = sqrt(beta_var)
		# Mean squared error
		mse = mean((data[[estvarname]] - true) ^ 2, na.rm = na.rm)
		...
		cover = mean(100 * (abs(data[[estvarname]] - true) < crit * data[[se]]), na.rm = na.rm)
		# Power of a significance test at the `level` level
		power = mean(100 * (abs(data[[estvarname]]) >= crit * data[[se]]), na.rm = na.rm)

		### Compute Monte Carlo SEs if requested:
		if (mcse) {
			bias_mcse = sqrt(beta_var / bsims)
			esd_mcse = esd / sqrt(2 * (bsims - 1))
			mse_mcse = sqrt(stats::var((data[[estvarname]] - true) ^ 2, na.rm = na.rm)) / sqrt(bsims)
			if (!is.null(esd_ref) & !is.null(rho) & !is.null(ncorr)) {
