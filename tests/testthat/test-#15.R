...
testthat::test_that("simsum with 'true' as a column returns the same results as using a scalar value if each row has the same value", {
  ...
  s1 <- simsum(data = tt, estvarname = "diff", true = "true", se = "se", ci.limits = c("conf.low", "conf.high"), methodvar = "method", by = "dgm")
  s2 <- simsum(data = tt, estvarname = "diff", true = -1, se = "se", ci.limits = c("conf.low", "conf.high"), methodvar = "method", by = "dgm")
...
testthat::test_that("multisimsum with 'true' as a column returns the same results as using a scalar value if each row has the same value", {
  ...
  m1 <- multisimsum(data = tt, estvarname = "diff", par = "par", true = c(diff1 = -1, diff2 = 0), se = "se", ci.limits = c("conf.low", "conf.high"), methodvar = "method", by = "dgm")
  m2 <- multisimsum(data = tt, estvarname = "diff", par = "par", true = "true", se = "se", ci.limits = c("conf.low", "conf.high"), methodvar = "method", by = "dgm")
