...
We compare the Cox model with a flexible parametric model with 2 degrees of freedom for modelling the baseline hazard (Royston and Parmar, 2002). The Cox model can be fit via the `coxph` function from the `survival` package, while the Royston-Parmar model can be fixed via the `stpm2` function from the `rstpm2` package.
