...
dgms <- dgms %>%
  dplyr::full_join(
    ...
    tibble::tibble(
      baseline = forcats::as_factor(
        x = c("E", "W", "WW1", "WW2", "WW3", "WW4")
      ),
      pars = list(
        list(lambda = 0.5),
    ...
    res.in <- foreach::foreach(
    ...
    ) %do%
      {
        ...
        if (dgms$baseline[i] == "E") {
          S <- simsurv::simsurv(
            dist = "exponential",
            lambdas = dgms$pars[[i]][["lambda"]],
            x = X,
            betas = c(trt = dgms$beta[i], epsilon = 1),
            maxt = 10
          )
        ...
        out$t[out$t < 1 / 365.242] <- 1 /
          365.242 +
          rnorm(length(out$t[out$t < 1 / 365.242]), mean = 0, sd = 1e-4)
        out$t <- abs(out$t)

        # Fit models
        f1 <- summary(survival::coxph(
          formula = survival::Surv(time = t, event = d) ~ trt,
          data = out
        ))
        f2 <- summary(rstpm2::stpm2(
          formula = survival::Surv(time = t, event = d) ~ trt,
          data = out,
          df = 5
        ))
        ...
        res <- tibble::tibble(
          ...
          se = c(
            f1$coefficients["trt", "se(coef)"],
            f2@coef["trt", "Std. Error"]
          )
        )
